...
describe('city shell trace replay gate', () => {
  ...
  it('uses model discriminators to keep all detailed labs exclusive', () => {
    ...
    const tiflashTrace = simulation.runScenario('tiflash-mpp')
    const tiflashEvent = tiflashTrace.events.find(
    ...
    expect(tiflashEvent).toBeDefined()
    ...
    expect(transaction.tiflashMpp.mode).toBe('hidden')
    ...
    expect(lock.tiflashMpp.mode).toBe('hidden')
    ...
    expect(raft.tiflashMpp.mode).toBe('hidden')
    ...
    expect(protocol.tiflashMpp.mode).toBe('hidden')
    ...
    expect(gc.tiflashMpp.mode).toBe('hidden')

    const tiflash = projectCityLabs(tiflashEvent!, true, true, 0.5)
    expect(tiflash.transaction.mode).toBe('hidden')
    expect(tiflash.lock.mode).toBe('hidden')
    expect(tiflash.raft.mode).toBe('hidden')
    expect(tiflash.protocol.mode).toBe('hidden')
    expect(tiflash.gcStorage.mode).toBe('hidden')
    expect(tiflash.tiflashMpp.mode).toBe('inspect')
    expect(tiflash.tiflashMpp.reducedMotion).toBe(true)
    expect(tiflash.tiflashMpp.learners.filter((learner) => learner.visible))
      .toHaveLength(3)
    expect(tiflash.tiflashMpp.tasks.filter((task) => task.visible))
      .toHaveLength(4)
    expect(tiflash.tiflashMpp.tunnels.filter((tunnel) => tunnel.visible))
      .toHaveLength(6)

    const closed = projectCityLabs(tiflashEvent!, false, true, 0.5)
    ...
    expect(closed.tiflashMpp.mode).toBe('hidden')
    ...
    expect(closed.tiflashMpp.reducedMotion).toBe(true)
  })
})
